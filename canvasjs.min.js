/* 
 CanvasJS Chart - v3.7.24 GA - https://canvasjs.com/ 
...
s,T=O?O.shortMonths:ha;O="";var V=!1;v=v&&v.getTime?v:v?new Date(v):new Date;if(isNaN(v))throw SyntaxError("invalid date");"UTC:"===J.slice(0,4)&&(J=J.slice(4),V=!0);O=V?"getUTC":"get";var G=v[O+"Date"](),M=v[O+"Day"](),W=v[O+"Month"](),a=v[O+"FullYear"](),d=v[O+"Hours"](),c=v[O+"Minutes"](),b=v[O+"Seconds"](),e=v[O+"Milliseconds"](),f=V?0:v.getTimezoneOffset();return O=J.replace(h,function(l){switch(l){case "D":return G;case "DD":return ca(G,2);case "DDD":return P[M];case "DDDD":return K[M];case "M":return W+
...
0>h;if(1>Math.abs(h)){var s=parseInt(h.toString().split("e-")[1]);s&&(h=(n?-1*h:h)*Math.pow(10,s-1),h="0."+Array(s).join("0")+h.toString().substring(2),h=n?"-"+h:h)}else s=parseInt(h.toString().split("+")[1]),20<s&&(s-=20,h/=Math.pow(10,s),h=h.toString()+Array(s+1).join("0"));return String(h)},da=function(h,n,s){if(null===h)return"";if(!isFinite(h))return h;h=Number(h);var w=0>h?!0:!1;w&&(h*=-1);var B=s?s.decimalSeparator:".",v=s?s.digitGroupSeparator:",",A="";n=String(n);var A=1,J=s="",K=-1,O=[],
...
1),Q=0===h?0:0===n?-(P+J):nb(n).length-P,A/=Math.pow(10,Q));0>K&&(K=M);A=(h*A).toFixed(T);n=A.split(".");A=(n[0]+"").split("");h=(n[1]+"").split("");A&&"0"===A[0]&&A.shift();for(G=J=M=T=K=0;0<O.length;)if(n=O.pop(),"#"===n||"0"===n)if(K++,K===P){var W=A,A=[];if("0"===n)for(n=P-T-(W?W.length:0);0<n;)W.unshift("0"),n--;for(;0<W.length;)s=W.pop()+s,G++,0===G%J&&(M===V&&0<W.length)&&(s=v+s)}else 0<A.length?(s=A.pop()+s,T++,G++):"0"===n&&(s="0"+s,T++,G++),0===G%J&&(M===V&&0<A.length)&&(s=v+s);else"E"!==
n[0]&&"e"!==n[0]||"0"!==n[n.length-1]||!/[eE][+-]*[0]+/.test(n)?","===n?(M++,J=G,G=0,0<A.length&&(s=v+s)):s=1<n.length&&('"'===n[0]&&'"'===n[n.length-1]||"'"===n[0]&&"'"===n[n.length-1])?n.slice(1,n.length-1)+s:n+s:(n=0>Q?n.replace("+","").replace("-",""):n.replace("-",""),s+=n.replace(/[0]+/,function(a){return ca(Q,a.length)}));v="";for(O=!1;0<X.length;)n=X.shift(),"#"===n||"0"===n?0<h.length&&0!==Number(h.join(""))?(v+=h.shift(),O=!0):"0"===n&&(v+="0",O=!0):1<n.length&&('"'===n[0]&&'"'===n[n.length-
1]||"'"===n[0]&&"'"===n[n.length-1])?v+=n.slice(1,n.length-1):"E"!==n[0]&&"e"!==n[0]||"0"!==n[n.length-1]||!/[eE][+-]*[0]+/.test(n)?v+=n:(n=0>Q?n.replace("+","").replace("-",""):n.replace("-",""),v+=n.replace(/[0]+/,function(a){return ca(Q,a.length)}));s+=(O?B:"")+v;return w?"-"+s:s},Pa=function(h){var n=0,s=0;h=h||window.event;h.offsetX||0===h.offsetX?(n=h.offsetX,s=h.offsetY):h.layerX||0==h.layerX?(n=h.layerX,s=h.layerY):(n=h.pageX-h.target.offsetLeft,s=h.pageY-h.target.offsetTop);return{x:n,y:s}},
ab=!0,Sa=window.devicePixelRatio||1,Oa=1,la=ab?Sa/Oa:1,Y=function(h,n,s,w,v,B,A,J,K,O,X,T,P){"undefined"===typeof P&&(P=1);A=A||0;J=J||"black";var G=15<w-n&&15<v-s?8:0.35*Math.min(w-n,v-s);h.beginPath();h.moveTo(n,s);h.save();h.fillStyle=B;h.globalAlpha=P;h.fillRect(n,s,w-n,v-s);h.globalAlpha=1;0<A&&(P=0===A%2?0:0.5,h.beginPath(),h.lineWidth=A,h.strokeStyle=J,h.moveTo(n,s),h.rect(n-P,s-P,w-n+2*P,v-s+2*P),h.stroke());h.restore();!0===K&&(h.save(),h.beginPath(),h.moveTo(n,s),h.lineTo(n+G,s+G),h.lineTo(w-
...
(h.textBlock.height-h.textBlock.fontSize/2)*Math.sin(Math.PI/180*Math.abs(h.textBlock.angle))),!a[g]||(!a[g].labelAutoFit||s(D)||s(B)||m.isNavigator||m.stockChart)||(h=0,0<a[g].labelAngle?B+u>n&&(h+=0<a[g].labelAngle?B+u-n-aa:0):0>a[g].labelAngle?D-v<l&&D-v<a[g].viewportMinimum&&(E=l-(N+a[g].tickLength+z+D-v+a[g].labelFontSize/2)):0===a[g].labelAngle&&(B+u>n&&(h=B+u/2-n-aa),D-v<l&&D-v<a[g].viewportMinimum&&(E=l-N-a[g].tickLength-z-D+v/2)),a[g].viewportMaximum===a[g].maximum&&a[g].viewportMinimum===
...
(q[g].calculateValueToPixelConversionParameters(),q[g].calculateBreaksSizeInValues());if(0<w){if(a&&0<a.length)for(g=0;g<a.length;g++)r=M[w-1][g]===M[w][g]?!0:!1;else r=!0;if(d&&0<d.length)for(g=0;g<d.length;g++)G=Q[w-1][g]===Q[w][g]?!0:!1;else G=!0;if(c&&0<c.length)for(g=0;g<c.length;g++)J=O[w-1][g]===O[w][g]?!0:!1;else J=!0;if(b&&0<b.length)for(g=0;g<b.length;g++)K=P[w-1][g]===P[w][g]?!0:!1;else K=!0}if(r&&G&&J&&K)break;w++}if(a&&0<a.length)for(g=0;g<a.length;g++)a[g].calculateStripLinesThicknessInValues(),
...
"cross"===b&&(c.strokeStyle=f,c.lineWidth=e/4,c.beginPath(),c.moveTo(a-e/2,d-e/2),c.lineTo(a+e/2,d+e/2),c.stroke(),c.moveTo(a+e/2,d-e/2),c.lineTo(a-e/2,d+e/2),c.stroke())}},drawMarkers:function(a){for(var d=0;d<a.length;d++){var c=a[d];W.drawMarker(c.x,c.y,c.ctx,c.type,c.size,c.color,c.borderColor,c.borderThickness)}}};return n}();B.version="v3.7.24 GA";window.CanvasJS&&(B&&!window.CanvasJS.Chart)&&(window.CanvasJS.Chart=B)})();
